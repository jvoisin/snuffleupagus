...
PHP_FUNCTION(sp_setcookie) {
   ...
   * javascript anyway.
   */
  if (zend_hash_exists(SNUFFLEUPAGUS_G(config).config_cookie->cookies,
		       name) > 0) {
    cookie_node = zend_hash_find_ptr(SNUFFLEUPAGUS_G(config).config_cookie->cookies,
						  name);
    if (cookie_node->encrypt) {
      httponly = 1;
    }
  }
